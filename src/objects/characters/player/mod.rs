    ...
    let cursor_world_position = match window.cursor_position() {
        Some(viewport_position) => {
            camera
                ...
                + player_velocity * fixed_time.delta_secs()
        }
        None => return Ok(()),
    };
...
// TODO: move this to the character controller with a condition on if the character has a
// camera or not
fn camera_tracking(
    fixed_time: Res<Time<Fixed>>,
    ...
    camera.translation =
        player_transform.translation + player_velocity.extend(0.0) * fixed_time.delta_secs();
}
